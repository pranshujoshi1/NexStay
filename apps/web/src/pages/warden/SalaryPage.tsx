...
export default function WardenSalaryPage() {
  const { data, isLoading } = useQuery({
    queryKey: ['warden-salary'],
    queryFn: () => api.get('/warden/salary').then(r => r.data.data),
  });
  ...
  return (
    <div>
      ...
      {staffRecord && (
        <div style={{ background: 'linear-gradient(135deg, #1e3a5f, #1e40af)', borderRadius: 16, padding: '24px', marginBottom: 20, color: 'white' }}>
          <p style={{ margin: '0 0 4px', opacity: 0.6, fontSize: 12, textTransform: 'uppercase', letterSpacing: 1 }}>Monthly Salary</p>
          <p style={{ margin: '0 0 16px', fontSize: 32, fontWeight: 700 }}>₹{staffRecord.salary?.toLocaleString('en-IN')}</p>
          <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap' }}>
            <div><p style={{ opacity: 0.6, fontSize: 11, margin: '0 0 2px' }}>Role</p><p style={{ margin: 0, fontSize: 14, fontWeight: 600 }}>{staffRecord.role}</p></div>
            <div><p style={{ opacity: 0.6, fontSize: 11, margin: '0 0 2px' }}>Joined</p><p style={{ margin: 0, fontSize: 14, fontWeight: 600 }}>{staffRecord.joiningDate ? new Date(staffRecord.joiningDate).toLocaleDateString('en-IN') : '—'}</p></div>
          </div>
        </div>
      )}
